...
export class FriendRequestService {
  ...
  async sendOrCancelFriendRequest(request_id: number, addressee_id: number) {
    ...
    const friendShip = await this.friendShipRepository.findOne({
      where: [
        { user_id: request_id, friend_id: addressee_id, deleteAt: IsNull() },
    ...
    if (friendRequest?.addressee_id == request_id) {
      throw new BadRequestException('Đã tồn tại lời mời kết bạn giữa 2 bạn');
    }
